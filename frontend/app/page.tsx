...
import { isDiscordActivity, getChannelId, getChannelName, subscribeToParticipants } from '@/lib/discordActivity';
...
function Dashboard() {
  ...
  const handleDiscordPlay = async () => {
    ...
    try {
      ...
      setDiscordError(err instanceof Error ? err.message : 'Failed to create room');
          ...
          <div className="w-full max-w-lg mx-auto space-y-4">
            <div className="bg-white/[0.01] backdrop-blur-xl border border-white/5 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="flex items-center gap-2.5 mb-4 pb-4 border-b border-white/5">
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-bold text-foreground/90 truncate">{channelName || 'Discord Voice'}</h3>
                  <p className="text-[10px] text-foreground/40 font-semibold">{participants.length} connected</p>
                </div>
              </div>
              ...
              {discordError && (
                <p className="text-red-400 text-[11px] text-center font-medium bg-red-500/5 rounded-xl px-4 py-2.5 mb-3">{discordError}</p>
              )}
              ...
              >
                ...
                {discordLoading ? 'Joining...' : 'Play with Voice Channel'}
              </button>
            </div>

            <div className="flex items-center gap-4">
              <div className="flex-1 h-px bg-white/5" />
              <span className="text-[10px] font-black uppercase text-foreground/30">OR USE CODE</span>
              <div className="flex-1 h-px bg-white/5" />
            </div>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="CODE"
                maxLength={4}
                className="flex-1 px-4 py-3 bg-white/5 border border-white/5 rounded-2xl text-center text-lg font-black tracking-[0.2em] text-[var(--accent)] placeholder-foreground/20 focus:outline-none focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)]/30 transition-all uppercase"
              />
              <button
                onClick={handleJoin}
