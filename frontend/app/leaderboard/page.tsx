...
export default function LeaderboardPage() {
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-black/10">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-foreground/60 text-sm font-medium animate-pulse">Loading Leaderboard...</p>
        </div>
      </div>
    );
  }
  ...
  if (displayedEntries.length > 1) {
    podiumItems.push({
      ...
      ringColor: 'ring-foreground/40 shadow-foreground/10',
      bg: 'from-foreground/20 to-background/40 border-foreground/20',
      height: 'h-32 md:h-36',
      badge: '🥈',
      delay: 0.1,
    });
  ...
  if (displayedEntries.length > 0) {
    podiumItems.push({
      ...
      bg: 'from-yellow-500/25 to-background/40 border-yellow-500/30',
      height: 'h-40 md:h-44',
      badge: '👑',
      isGold: true,
      delay: 0,
  ...
  if (displayedEntries.length > 2) {
    podiumItems.push({
      ...
      bg: 'from-amber-600/20 to-background/40 border-amber-600/20',
      height: 'h-24 md:h-28',
      badge: '🥉',
      delay: 0.2,
    });
  ...
  return (
    <div className="flex-1 flex flex-col p-4 md:p-8 max-w-5xl mx-auto w-full gap-6 min-h-0 overflow-y-auto">
            ...
            <div className="inline-flex p-1 bg-white/[0.02] border border-white/5 rounded-xl backdrop-blur-md shadow-inner">
              <button
                ...
                className={`px-4 py-1.5 text-xs font-extrabold rounded-lg transition-all duration-200 cursor-pointer ${
                  !showFriendsOnly
                    ? 'bg-[var(--primary)] text-foreground shadow shadow-[var(--primary)]/20'
        ...
        <div className="flex-1 flex flex-col gap-6">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative mb-3">
                        ...
                        <div className={`w-14 h-14 md:w-16 md:h-16 rounded-full bg-surface-light flex items-center justify-center text-lg font-bold overflow-hidden ring-4 ${item.ringColor} transition-transform group-hover:scale-105 duration-200 relative`}>
                          {entry.avatar_url ? (
                        ...
                        <span className="absolute -bottom-1 -right-1 bg-background text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center border border-white/10 shadow shadow-black">
                          {item.badge}
                        </span>
                      </div>

                      {/* Info & Score preview */}
                      <span className="font-bold text-xs md:text-sm text-foreground/90 truncate w-full text-center group-hover:text-foreground transition-colors">
          ...
          {listEntries.length > 0 && (
            <div className="bg-white/[0.01] border border-white/5 rounded-3xl p-4 shadow-xl flex-1 flex flex-col space-y-3">
              <h3 className="text-xs font-bold text-foreground/40 uppercase tracking-widest px-2">Runner Ups</h3>
              <div className="space-y-2 max-h-[360px] overflow-y-auto pr-1">
                {listEntries.map((e, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span className="w-6 text-center text-xs font-extrabold text-foreground/40 tabular-nums">
                        #{rank}
                      </span>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-sm text-foreground/90 truncate">{e.username || e.player_name || 'Unknown'}</p>
                        <p className="text-[10px] text-foreground/40 font-semibold">{e.games_played} games · {e.wins || 0} wins</p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-base font-black text-[var(--accent)] tabular-nums">{e.total_score.toLocaleString()}</p>
                        <p className="text-[9px] font-bold text-foreground/30 uppercase tracking-wider">pts</p>
                      </div>
                    </motion.div>
                  );
                })}
        ...
        <AnimatePresence mode="wait">
          {selectedEntry && (
            <motion.div
              ...
              className="w-full lg:w-80 shrink-0 bg-background/60 backdrop-blur-xl rounded-3xl border border-white/10 p-6 flex flex-col justify-between shadow-2xl relative overflow-hidden min-h-[420px]"
            >
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 border-b border-white/5 pb-4">
                  <div className="relative">
                    <div className="w-14 h-14 rounded-full bg-surface-light flex items-center justify-center text-xl font-black overflow-hidden border-2 border-[var(--primary)] shadow-md shadow-[var(--primary)]/20">
                    ...
                    <span className="absolute -bottom-1 -right-1 bg-[var(--primary)] text-[9px] font-black uppercase px-1.5 py-0.5 rounded-md text-foreground border border-black">
                      #{entries.findIndex(e => e.id === selectedId) + 1}
                    </span>
                  </div>
                  <div className="min-w-0">
                    <p className="font-extrabold text-base text-foreground truncate">{selectedEntry.username || selectedEntry.player_name || 'Unknown'}</p>
                    <p className="text-[10px] text-foreground/40 font-bold uppercase tracking-wider mt-0.5">Player Statistics</p>
                  </div>
                </div>
                ...
                {statsLoading ? (
                  <div className="flex flex-col items-center justify-center py-12 gap-2">
                    ...
                    <p className="text-xs text-foreground/40 font-medium">Fetching details...</p>
                  </div>
                ) : selectedStats ? (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3 text-center transition-all hover:bg-white/[0.04] hover:border-white/10">
                      ...
                      <p className="text-[9px] font-bold text-foreground/40 uppercase tracking-widest mt-1">Total Points</p>
                    </div>
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3 text-center transition-all hover:bg-white/[0.04] hover:border-white/10">
                      <p className="text-lg font-black text-[var(--primary)] tabular-nums">{selectedStats.gamesPlayed ?? '-'}</p>
                      <p className="text-[9px] font-bold text-foreground/40 uppercase tracking-widest mt-1">Games Played</p>
                    </div>
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3 text-center transition-all hover:bg-white/[0.04] hover:border-white/10">
                      <p className="text-lg font-black text-foreground tabular-nums">{selectedStats.bestScore ?? '-'}</p>
                      <p className="text-[9px] font-bold text-foreground/40 uppercase tracking-widest mt-1">Best Score</p>
                    </div>
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3 text-center transition-all hover:bg-white/[0.04] hover:border-white/10">
                      <p className="text-lg font-black text-foreground tabular-nums">{selectedStats.perfects ?? '-'}</p>
                      <p className="text-[9px] font-bold text-foreground/40 uppercase tracking-widest mt-1">Perfect Rounds</p>
                    </div>
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3 text-center transition-all hover:bg-white/[0.04] hover:border-white/10">
                      <p className="text-lg font-black text-foreground tabular-nums">{selectedStats.totalRounds ?? '-'}</p>
                      <p className="text-[9px] font-bold text-foreground/40 uppercase tracking-widest mt-1">Rounds Played</p>
                    </div>
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3 text-center transition-all hover:bg-white/[0.04] hover:border-white/10 overflow-hidden">
                      <p className="text-xs font-black text-foreground capitalize truncate mt-0.5">{selectedStats.bestGenre ?? '-'}</p>
                      <p className="text-[9px] font-bold text-foreground/40 uppercase tracking-widest mt-2">Best Genre</p>
                    </div>
                    {selectedStats.averageSpeedMs != null && (
                      <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-3.5 text-center col-span-2 transition-all hover:bg-white/[0.04] hover:border-white/10">
                        ...
                        <p className="text-[9px] font-bold text-foreground/40 uppercase tracking-widest mt-1">Average Answer Speed</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <p className="text-xs text-foreground/40 text-center py-12">No detailed stats available</p>
                )}
              </div>

              <button
                onClick={() => setSelectedId(null)}
                className="mt-6 w-full py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-foreground/80 hover:text-foreground rounded-xl text-xs font-bold transition-all cursor-pointer text-center"
              >
                Close Details
              </button>
            </motion.div>
      ...
      <Link href="/" className="text-xs text-foreground/40 hover:text-foreground/80 text-center transition-colors font-semibold uppercase tracking-wider mt-4">
        Back Home
      </Link>
    </div>
  );
