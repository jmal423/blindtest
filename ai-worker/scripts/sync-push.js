...
const AI_COLS = ['ai_genres', 'ai_tags', 'ai_audio_genres', 'ai_confidence', 'ai_processed_at', 'ai_version'];
...
async function pullUnsyncedBatch(offset) {
  const { rows } = await local.query(`
...
for (let offset = 0; offset < total; offset += BATCH_SIZE) {
  const rows = await pullUnsyncedBatch(offset);
